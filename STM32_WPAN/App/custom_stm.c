...
typedef struct{
  ...
  uint16_t  CustomButton_CHdle;                  /**< My_Button_Char handle */
...
#define COPY_MY_BUTTON_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x42,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        ...
        case ACI_GATT_WRITE_PERMIT_REQ_VSEVT_CODE:
          ...
          if (write_perm_req->Attribute_Handle == (CustomContext.CustomLed_CHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
...
void SVCCTL_InitCustomSvc(void)
{
  ...
  ret = aci_gatt_add_char(CustomContext.CustomMy_P2PsHdle,
                          ...
                          &(CustomContext.CustomButton_CHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : BUTTON_C, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : BUTTON_C \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_BUTTON_C:
      ret = aci_gatt_update_char_value(CustomContext.CustomMy_P2PsHdle,
                                       CustomContext.CustomButton_CHdle,
                                       0, /* charValOffset */
                                       SizeButton_C, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value BUTTON_C command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value BUTTON_C command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_BUTTON_C:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMy_P2PsHdle, CustomContext.CustomButton_CHdle, SizeButton_C, pPayload);
